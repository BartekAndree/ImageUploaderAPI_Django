...
class FileListSerializer(serializers.ModelSerializer):
    ...
    def get_thumbnail_url_200px(self, obj):
        ...
        if build_in_tier in ['basic', 'premium', 'enterprise']:
            ...
            img = PILImage.open(obj.image_url) 

            height = 200
            ratio = (height / float(img.size[1]))
            img = img.resize(
                (int((float(img.size[0]) * float(ratio))), height),
                  PILImage.ANTIALIAS
                  )
            ...
            img.save(f'{BASE}/mediafiles/images/{file_name}')
    ...
    def get_arbitary_thumbnail(self, obj):
        ...
        if arbitary_tier is not None:
            ...
            height = user.arbitary_tier.thumbnail_size
            ratio = (height / float(img.size[1]))
            img = img.resize(
                (int((float(img.size[0]) * float(ratio))), height),
                  PILImage.ANTIALIAS
            ...
            name = file_name.split(".")[0]
            exten = file_name.split(".")[-1]
            file_name = f"{name}_{height}px.{exten}"
...
class FileUploadPrimaryKeyRelatedField(serializers.PrimaryKeyRelatedField):
    def get_queryset(self):
        ...
        return FileUpload.objects.filter(creator = user)
